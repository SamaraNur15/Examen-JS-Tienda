...
async function cargarProductos() {
    ...
    try{
        ...
        if(gridProductos){
            gridProductos.innerHTML = '<p class="error">No se pudieron cargar los productos. Intente nuevamente más tarde.</p>';
        }
    }    
}
/**
...
function mostrarProductos(productos) {
    gridProductos.innerHTML = ''; // Paso 1: Limpiar contenido anterior
    if (!Array.isArray(productos) || productos.length === 0) {
        gridProductos.innerHTML = '<p class="info">No hay productos disponibles.</p>';
        return;
    }
    productos.forEach(producto => { // Paso 2: Recorrer array de productos  
        const card = document.createElement('div'); // Paso 3: Crear tarjeta para cada producto
        ...
        `;
        gridProductos.appendChild(card); // Paso 4: Agregar tarjeta al grid
    });
...
function agregarAlCarrito(idProducto) {
    ...
    const existe = carritoLs.find(item => item.id === idProducto);
...
function actualizarContadorCarrito() {
    const carritoLs = JSON.parse(localStorage.getItem('carrito')) || [];
    const total = carritoLs.reduce((acc, item) => acc + (item.cantidad || 0), 0);
    if (contadorCarrito) {
